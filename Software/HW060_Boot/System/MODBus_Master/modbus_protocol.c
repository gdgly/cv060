...
u16 MB_Get_Holding_Register(u16 i)
{
	...
	case 2:		tmp16=NUM_PID; 					return tmp16;	// PID blocks number
	case 3:		tmp16=MAX_FANCOIL_NUMBER; 		return tmp16;	// FAN blocks number
	case 4:		tmp16=MAX_KOTEL_NUMBER;			return tmp16;	// KOTEL blocks number
	case 5:		tmp16=MAX_VALVE_3T_NUMBER;		return tmp16;	// VALVE blocks number
	case 6:		tmp16=MAX_PARAMS_BLOCK;			return tmp16;	// PARAMS blocks number
	
	case 10:	tmp16=mb_pid_number;			return tmp16;	// active pid number
	case 11:	tmp16=mb_fan_number;			return tmp16;	// active fan number
	case 12:	tmp16=mb_kot_number;			return tmp16;	// active kot number
	case 13:	tmp16=mb_valve_number;			return tmp16;	// active valve number
	case 14:	tmp16=mb_param_number;			return tmp16;	// active param number

	// PID block
#if NUM_PID
	case 20+0:	tmp16=0; if(mb_pid_number < NUM_PID) tmp16 = pid[mb_pid_number].channel;				return tmp16;	//
	case 20+1:	tmp16=0; if(mb_pid_number < NUM_PID) tmp16 = pid[mb_pid_number].p;						return tmp16;	//
	case 20+2:	tmp16=0; if(mb_pid_number < NUM_PID) tmp16 = pid[mb_pid_number].i;						return tmp16;	//
	case 20+3:	tmp16=0; if(mb_pid_number < NUM_PID) tmp16 = pid[mb_pid_number].d;						return tmp16;	//
	case 20+4:	tmp16=0; if(mb_pid_number < NUM_PID) tmp16 = pid[mb_pid_number].t0;						return tmp16;	//
	case 20+5:	tmp16=0; if(mb_pid_number < NUM_PID) tmp16 = pid[mb_pid_number].power;					return tmp16;	//
	case 20+6:	tmp16=0; if(mb_pid_number < NUM_PID) tmp16 = pid[mb_pid_number].power_min;				return tmp16;	//
	case 20+7:	tmp16=0; if(mb_pid_number < NUM_PID) tmp16 = pid[mb_pid_number].power_max;				return tmp16;	//
	case 20+8:	tmp16=0; if(mb_pid_number < NUM_PID) tmp16 = pid[mb_pid_number].mode;					return tmp16;	//
	case 20+9:	tmp16=0; if(mb_pid_number < NUM_PID) tmp16 = pid[mb_pid_number].tin;					return tmp16;	//
	case 20+10:	tmp16=0; if(mb_pid_number < NUM_PID) tmp16 = pid[mb_pid_number].PID_POWER_STEP[0]*1000;	return tmp16;	//
	case 20+11:	tmp16=0; if(mb_pid_number < NUM_PID) tmp16 = pid[mb_pid_number].PID_POWER_STEP[1]*1000;	return tmp16;	//
	case 20+12:	tmp16=0; if(mb_pid_number < NUM_PID) tmp16 = pid[mb_pid_number].PID_POWER_LIMIT[0];		return tmp16;	//
	case 20+13:	tmp16=0; if(mb_pid_number < NUM_PID) tmp16 = pid[mb_pid_number].PID_POWER_LIMIT[1];		return tmp16;	//
	case 20+14:	tmp16=0; if(mb_pid_number < NUM_PID) tmp16 = pid[mb_pid_number].heating_cooling;		return tmp16;	//
	case 20+15:	tmp16=0; if(mb_pid_number < NUM_PID) tmp16 = pid[mb_pid_number].fan_in;					return tmp16;	//
	case 20+16:	tmp16=0; if(mb_pid_number < NUM_PID) tmp16 = pid[mb_pid_number].fan_out;				return tmp16;	// 	
#endif
	...
	// Params Block
#if MAX_PARAMS_BLOCK
	case 250+0:	tmp16=0; if(mb_param_number < MAX_PARAMS_BLOCK) tmp16 = misc.pr[0];	return tmp16;		//
	case 250+1:	tmp16=0; if(mb_param_number < MAX_PARAMS_BLOCK) tmp16 = misc.pr[1];	return tmp16;		//
	case 250+2:	tmp16=0; if(mb_param_number < MAX_PARAMS_BLOCK) tmp16 = misc.pr[2];	return tmp16;		//
	case 250+3:	tmp16=0; if(mb_param_number < MAX_PARAMS_BLOCK) tmp16 = misc.pr[3];	return tmp16;		//
	case 250+4:	tmp16=0; if(mb_param_number < MAX_PARAMS_BLOCK) tmp16 = misc.pr[4];	return tmp16;		//
	case 250+5:	tmp16=0; if(mb_param_number < MAX_PARAMS_BLOCK) tmp16 = misc.pr[5];	return tmp16;		//
	case 250+6:	tmp16=0; if(mb_param_number < MAX_PARAMS_BLOCK) tmp16 = misc.pr[6];	return tmp16;		//
	case 250+7:	tmp16=0; if(mb_param_number < MAX_PARAMS_BLOCK) tmp16 = misc.pr[7];	return tmp16;		//
	case 250+8:	tmp16=0; if(mb_param_number < MAX_PARAMS_BLOCK) tmp16 = misc.pr[8];	return tmp16;		//
	case 250+9:	tmp16=0; if(mb_param_number < MAX_PARAMS_BLOCK) tmp16 = misc.pr[9];	return tmp16;		//
#endif		
	default: return 0;
	}
}			
...
//	case 6:		tmp16=MAX_PARAMS_BLOCK;			return;	// PARAMS blocks number
	...
	case 10:	mb_pid_number = value;			return;	// active pid number
	case 11:	mb_fan_number=value;			return;	// active fan number
	case 12:	mb_kot_number=value;			return;	// active kot number
	case 13:	mb_valve_number=value;			return;	// active valve number
	case 14:	mb_param_number=value;			return;	// active param number

	// PID block
#if NUM_PID
	case 20+0:	if(mb_pid_number < NUM_PID) pid[mb_pid_number].channel = value;					return;	//
	case 20+1:	if(mb_pid_number < NUM_PID) pid[mb_pid_number].p = value;						return;	//
	case 20+2:	if(mb_pid_number < NUM_PID) pid[mb_pid_number].i = value;						return;	//
	case 20+3:	if(mb_pid_number < NUM_PID) pid[mb_pid_number].d = value;						return;	//
	case 20+4:	if(mb_pid_number < NUM_PID) pid[mb_pid_number].t0 = value;						return;	//
	case 20+5:	if(mb_pid_number < NUM_PID) pid[mb_pid_number].power = value;					return;	//
	case 20+6:	if(mb_pid_number < NUM_PID) pid[mb_pid_number].power_min = value;				return;	//
	case 20+7:	if(mb_pid_number < NUM_PID) pid[mb_pid_number].power_max = value;				return;	//
	case 20+8:	if(mb_pid_number < NUM_PID) pid[mb_pid_number].mode = value;					return;	//
	case 20+9:	if(mb_pid_number < NUM_PID) pid[mb_pid_number].tin = value;						return;	//
	case 20+10:	if(mb_pid_number < NUM_PID) pid[mb_pid_number].PID_POWER_STEP[0] = value/1000;	return;	//
	case 20+11:	if(mb_pid_number < NUM_PID) pid[mb_pid_number].PID_POWER_STEP[1] = value/1000;	return;	//
	case 20+12:	if(mb_pid_number < NUM_PID) pid[mb_pid_number].PID_POWER_LIMIT[0] = value;		return;	//
	case 20+13:	if(mb_pid_number < NUM_PID) pid[mb_pid_number].PID_POWER_LIMIT[1] = value;		return;	//
	case 20+14:	if(mb_pid_number < NUM_PID) pid[mb_pid_number].heating_cooling = value;			return;	//
	case 20+15:	if(mb_pid_number < NUM_PID) pid[mb_pid_number].fan_in = value;					return;	//
	case 20+16:	if(mb_pid_number < NUM_PID) pid[mb_pid_number].fan_out = value;					return;	// 	
#endif
	
	// Fancoils block
#if MAX_FANCOIL_NUMBER
	case 50+0:	if(mb_fan_number < MAX_FANCOIL_NUMBER) fan[mb_fan_number].T = value;					return;	//
	case 50+1:	if(mb_fan_number < MAX_FANCOIL_NUMBER) fan[mb_fan_number].Fan_Mode = value;				return;	//
	...
	// Params Block
#if MAX_PARAMS_BLOCK
	case 250+0:	if(mb_param_number < MAX_PARAMS_BLOCK) misc.pr[0] = value;	return;		//
	case 250+1:	if(mb_param_number < MAX_PARAMS_BLOCK) misc.pr[1] = value;	return;		//
	case 250+2:	if(mb_param_number < MAX_PARAMS_BLOCK) misc.pr[2] = value;	return;		//
	case 250+3:	if(mb_param_number < MAX_PARAMS_BLOCK) misc.pr[3] = value;	return;		//
	case 250+4:	if(mb_param_number < MAX_PARAMS_BLOCK) misc.pr[4] = value;	return;		//
	case 250+5:	if(mb_param_number < MAX_PARAMS_BLOCK) misc.pr[5] = value;	return;		//
	case 250+6:	if(mb_param_number < MAX_PARAMS_BLOCK) misc.pr[6] = value;	return;		//
	case 250+7:	if(mb_param_number < MAX_PARAMS_BLOCK) misc.pr[7] = value;	return;		//
	case 250+8:	if(mb_param_number < MAX_PARAMS_BLOCK) misc.pr[8] = value;	return;		//
	case 250+9:	if(mb_param_number < MAX_PARAMS_BLOCK) misc.pr[9] = value;	return;		//
#endif		
	default: return;
	}
}			
