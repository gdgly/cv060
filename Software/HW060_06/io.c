...
io_init(void)
{
    ...
    GPIOPinWrite(GPIO_PORTD_BASE, GPIO_PIN_5+GPIO_PIN_6+GPIO_PIN_7, 0);
	// Port E
...
//	PWMPulseWidthSet(PWM_BASE, PWM_OUT_2, ((ulPWMClock * 80)/100) / g_ulFrequency);
	...
	ADCSequenceStepConfigure(ADC0_BASE, 1, 0, ADC_CTL_CH0 | 0 | 0);
	ADCSequenceStepConfigure(ADC0_BASE, 1, 1, ADC_CTL_CH1 | 0 | 0);
	ADCSequenceStepConfigure(ADC0_BASE, 1, 2, ADC_CTL_CH2 | 0 | 0);
...
// Process IO DI
//------------------------------------------------------------------------------
unsigned long di_count [8] = {0};
unsigned long di_process(unsigned long old_di)
{
	unsigned long di = old_di;
	unsigned long di_new = 0;
	unsigned long adc_res[9]={0};
	int i;
	unsigned long tmp = 1;
		
	if(GPIOPinRead(GPIO_PORTF_BASE, GPIO_PIN_1)==0)di|=0x00000001; else di&=~0x00000001;
	if(GPIOPinRead(GPIO_PORTA_BASE, GPIO_PIN_7)==0)di|=0x00000002; else di&=~0x00000002;
	if(GPIOPinRead(GPIO_PORTA_BASE, GPIO_PIN_3)==0)di|=0x00000004; else di&=~0x00000004;
	if(GPIOPinRead(GPIO_PORTC_BASE, GPIO_PIN_4)==0)di|=0x00000008; else di&=~0x00000008;
...
//	if(GPIOPinRead(GPIO_PORTD_BASE, GPIO_PIN_1)==0)di|=0x00000080;
	...
	{
		ADCSequenceDataGet(ADC0_BASE, 1, adc_res);
		ADCIntClear(ADC0_BASE, 1);
		ADCProcessorTrigger(ADC0_BASE, 1);
		if(adc_res[0]<512)di|=0x00000080; else di&=~0x00000080;
		if(adc_res[1]<512)di|=0x00000040; else di&=~0x00000040;
		if(adc_res[2]<512)di|=0x00000020; else di&=~0x00000020;
		if(adc_res[3]<512)di|=0x00000010; else di&=~0x00000010;
	}
	...
	for (i=0; i<8; i++){
		if (di & tmp) {
			di_count[i] ++;
			if (di_count[i]>100) di_count[i] = 100;     
		} else {
			if (di_count[i]) di_count[i]--;
		}
		if (di_count[i]>50) di_new |= tmp;
		tmp = tmp << 1;
	}
	return di_new;
}
...
void ao_process(short __packed *ao ,unsigned char num)
{
	...
	AO[1]=*ao++;
	AO[2]=*ao++;
	ulPWMClock = SysCtlClockGet()/4000;
	for(i=0;i<num;i++)
	{
		if(AO[i]>20)AO[i]=AO[i]-(999-AO[i])/50;
		else AO[i]/=4;
		if(AO[i]>999)AO[i]=999;
		if(AO[i]==0)AO[i]=1;
	}
	if(num>0)PWMPulseWidthSet(PWM_BASE, PWM_OUT_0, ((ulPWMClock * AO[0])/1) / g_ulFrequency);
	if(num>1)PWMPulseWidthSet(PWM_BASE, PWM_OUT_1, ((ulPWMClock * AO[1])/1) / g_ulFrequency);
	if(num>2)PWMPulseWidthSet(PWM_BASE, PWM_OUT_2, ((ulPWMClock * AO[2])/1) / g_ulFrequency);
}
//-----------------------------------------------------------------------------------
...
// File load
//*****************************************************************************
u8 file_load(void *params, u32 size, char*  name)
{
	WORD rl;
...
//	IntMasterDisable();
	...
	fresult=f_read(&file,(u8*)params,size,&rl);				if(fresult != FR_OK) {f_close(&file);return 0;}
	f_close(&file);
//	IntMasterEnable();
...
//*****************************************************************************
u8 file_store(void *params, u32 size, char*  name)
{
	WORD rl;
...
//	IntMasterDisable();
	...
	fresult=f_write(&file,(u8*)params,size,&rl);			if(fresult != FR_OK) {f_close(&file);return 0;}
	f_close(&file);
//	IntMasterEnable();
