...
in_flash u8	fancoil_dic[][40]=
{
	...
	"\n\r     Difference             ", //32	
	"\n\r No fancoils", 				//33
	"\n\r Wrong fancoil number",		//34	
	"\n\r     T In                   ",	//35	
	"\n\r     HC Heating/Cooling     ",	//36	
	"\n\r     T Emeregency           ",	//37	
...
#if MAX_FANCOIL_NUMBER!=0	
	if(Params[0]>=MAX_FANCOIL_NUMBER)return;
	fan[Params[0]].Fan_Mode=Params[1];
#endif
}
//-----------------------------------------------------------------------------------
...
//	f32 ftmp;
	...
	if(Params[0]<MAX_FANCOIL_NUMBER)
	{
	//	Mod_Show_Full_Time();
		
	//	fan_temperature=CV.fan[num].Temperature;
		// Current state
...
//		__sprintf_P(outtxt,fancoil_dic[26]);if(fan[Params[0]].Day_Night==fan_Day)__sprintf_P(outtxt,fancoil_dic[27]);else __sprintf_P(outtxt,fancoil_dic[28]);			
		__sprintf_P(outtxt,fancoil_dic[22]);ShowTemperature(outtxt,(s16*)&fan[Params[0]].T);
		__sprintf_P(outtxt,fancoil_dic[35]);ShowTemperature(outtxt,(s16*)&fan[Params[0]].T_In);
		__sprintf_P(outtxt,fancoil_dic[36]);ByteToStr(outtxt,(u8*)&fan[Params[0]].HC,0);	
		
//		__sprintf_P(outtxt,fancoil_dic[31]);ByteToStr(outtxt,&fan[Params[0]].Channel,1);__sprintf(outtxt,"]     ");	
//		ShowTemperature(outtxt,(s16*)&io.ainput[fan[Params[0]].Channel]);	
		diff=fan[Params[0]].T_In-fan[Params[0]].T;
		if(diff<0)diff=-diff;
	...
	//	__sprintf_P(outtxt,fancoil_dic[2]);ShowTemperature(outtxt,&fan[Params[0]].Step);
		__sprintf_P(outtxt,fancoil_dic[3]);ShowTemperature(outtxt,(s16*)&fan[Params[0]].TLow);
		__sprintf_P(outtxt,fancoil_dic[4]);ShowTemperature(outtxt,(s16*)&fan[Params[0]].TMedium);
		__sprintf_P(outtxt,fancoil_dic[5]);ShowTemperature(outtxt,(s16*)&fan[Params[0]].THigh);
		__sprintf_P(outtxt,fancoil_dic[6]);ShowTemperature(outtxt,(s16*)&fan[Params[0]].Hysteresis);
	//	__sprintf_P(outtxt,fancoil_dic[7]);ShowTemperature(outtxt,&fan[Params[0]].TEmeregency);						
		__sprintf_P(outtxt,fancoil_dic[8]);WordToStr(outtxt,(u16*)&fan[Params[0]].Fan_Time,0);
//		__sprintf_P(outtxt,fancoil_dic[9]);WordToStr(outtxt,(u16*)&fan[Params[0]].Valve_Time_1,0);
//		__sprintf_P(outtxt,fancoil_dic[24]);WordToStr(outtxt,(u16*)&fan[Params[0]].Valve_Time_2,0);
		__sprintf_P(outtxt,fancoil_dic[25]);ShowTemperature(outtxt,(s16*)&fan[Params[0]].Valve_Hysteresis);		
	...
	//	__sprintf_P(outtxt,fancoil_dic[11]);ShowTemperature(outtxt,&io.ainput[1]);						
		...
		__sprintf_P(outtxt,fancoil_dic[34]);
#else
	__sprintf_P(outtxt,fancoil_dic[33]);
#endif
}
//-----------------------------------------------------------------------------------
void Fan_Lines_Update(u8 num)
{
#if MAX_FANCOIL_NUMBER!=0
	u8 fan_speed=0;

	if(fan[num].Fan_Lvl==0)
		Fan_Switch_Enable[num]=fan[num].Fan_Time;	

	switch(fan[num].Fan_Lvl)
	{
		case 0:
			fan_speed=0x00;
		break;
	...
	if(FanLines[num].fan_speed!=fan_speed)
	{
		if(FanLines[num].fan_speed==0)
		{
			if(Fan_Switch_Enable[num]==0)						// Timeout is over
			{
				FanLines[num].fan_speed=fan_speed;				// Setup relays
				Fan_Switch_Enable[num]=fan[num].Fan_Time;	// Set up timeout
			}
		}
		else
		{
			FanLines[num].fan_speed=0;
			Fan_Switch_Enable[num]=fan[num].Fan_Time;		// Set up timeout			
		}
	}
	//intr.out_params[19]=FanLines[num].fan_speed;
#endif
}
//-----------------------------------------------------------------------------------
...
/*
	if(SysSetUp!=0)	
	{
		...
		Fan_Switch_Enable[num]=fan[num].Fan_Time;		// Set up timeout
//		Valve_Switch_Enable[num]=fan[num].Valve_Time_1;	// Set up timeout
//		Valve_Transfer_Enable[num]=fan[num].Valve_Time_2;	// Set up timeout		
		return;				
	}	
*/	
	//Diff=io.ainput[fan[num].Channel]-fan[num].T;
	Diff = fan[num].T_In - fan[num].T;

	if (Diff < 0)
		if (fan[num].HC == 1) // Cooling
    		Diff = 0;
	
	if (Diff > 0)
		if (fan[num].HC == 0) // Heating
    		Diff = 0;
...
/*
#if MAX_SEASON_BLOCK
		if(reg_season.season==SEASON_SUMMER)Diff2=0;
#endif
	}
#if MAX_SEASON_BLOCK
	else
		if(reg_season.season==SEASON_WINTER)Diff2=0;
#endif
*/
	}
	
	//if(fan[num].Day_Night==fan_Day)	
	{
		if(fan[num].Fan_Mode==FAN_AUTO)
		{		
			switch(fan[num].Fan_Lvl)
			{
				case 0:
				...
				break;
				case 1:
					low=fan[num].TLow-fan[num].Hysteresis;
					med=fan[num].TMedium;
					high=fan[num].THigh;				
				break;
				case 2:
					low=fan[num].TLow-fan[num].Hysteresis;
					med=fan[num].TMedium-fan[num].Hysteresis;
					high=fan[num].THigh;							
				break;						
				case 3:
					low=fan[num].TLow-fan[num].Hysteresis;
					med=fan[num].TMedium-fan[num].Hysteresis;
					high=fan[num].THigh-fan[num].Hysteresis;							
				break;						
			}
			fan_lvl=0;
			...
			fan[num].Fan_Lvl = fan_lvl;
/*			
			if (fan[num].Fan_Lvl != fan_lvl){
				fan[num].Fan_Lvl = 0;	
				Fan_Switch_Enable[num] = fan[num].Fan_Time;	
			}
			//Fan_Lines_Update(num);
			if ((Fan_Switch_Enable[num] == 0) && (fan[num].Fan_Lvl == 0))
				fan[num].Fan_Lvl = fan_lvl;	
		
			switch(fan[num].Fan_Lvl)
			{
				case 0:
					FanLines[num].fan_speed=0x00;
				break;
				case 1:
					FanLines[num].fan_speed=0x01;
				break;
				case 2:
					FanLines[num].fan_speed=0x02;
				break;
				case 3:
					FanLines[num].fan_speed=0x04;
				break;
			}	
*/			
...
//			fan[num].Fan_Lvl=fan_lvl;					
	...
	/*	
		...
		if(fan[num].Fan_Lvl!=0)
		{
			...
			if((Diff>=fan[num].Valve_Hysteresis))
			{
				fan[num].Valve=2;				
			}
			else
			{
				if((Diff<=-fan[num].Valve_Hysteresis))	// Heating
				{
					fan[num].Valve=1;				
				}
				else
				{				
					// Turning off Valve
					if(Diff2<=(fan[num].Valve_Hysteresis/2))
					{
						fan[num].Valve=0;
					}
				}
...
{
	u8 i;
	for(i=0;i<MAX_FANCOIL_NUMBER;i++)Fancoil_Control(i);
}
//-----------------------------------------------------------------------------------
