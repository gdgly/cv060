...
//	tmp=0x8e;	// Enable whatchdog
	...
	tmp=0x19;	// 2.9V level reset & full mem protection
//	tmp=0x01;	// 2.9V level reset & full mem protection
...
//-----------------------------------------------------------------------------------
void fram_comp_read_no_int(u8 addr,u8* data, u16 len)
{
#if FRAM_ENABLE			
	Addr[0]=addr;
	Addr[1]=0;	
	I2C_NiMasterRead_NoInt(0xd2,Addr,1,data,len);	
#endif		
}
//-----------------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------------
void fram_get_time(TCTime *time)
{
	TCTime	ft;
	u8 tmp;
...
u16 CheckCRC(u8 who,u32 PARAM_LOCATION)
{
	u32 len,p,Addr;
	u16 crc;
	u8 ctmp;
