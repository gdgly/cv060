...
/*---------------------------------------------------------------------------*/
/**
  @brief	get 32 bits data from buf
  @param	var	a 32 bits variable
  @param	buf	a pointer to data buffer
  @return	none
 */
/*---------------------------------------------------------------------------*/
#define GETU32( var, buf) {				\
		(var)  = (*(buf) 	<< 24) 	& 0xFF000000;		\
		(var) |= (*(buf+1) 	<< 16) 	& 0x00FF0000;		\
		(var) |= (*(buf+2) 	<< 8)  	& 0x0000FF00;		\
		(var) |= (*(buf+3)  		& 0x000000FF);		\
	}
/*---------------------------------------------------------------------------*/
/**
...
/*---------------------------------------------------------------------------*/
/**
  @brief	get 8 bits data from buf
  @param	var	a 8 bits variable
  @param	buf	a pointer to data buffer
  @return	none
 */
/*---------------------------------------------------------------------------*/
#define GETU8( var, buf) {				\
		(var) = (*(buf)) & 0xFF;		\
	}
/*---------------------------------------------------------------------------*/
/**
...
/*---------------------------------------------------------------------------*/
/**
  @brief	put 8 bits data to buf
  @param	buf	a pointer to data buffer
  @param	var	a 8 bits variable
  @return	none
 */
/*---------------------------------------------------------------------------*/
#define PUTU8( buf, var) {				\
		(*(buf)) = ((var) & 0xFF);		\
	}
