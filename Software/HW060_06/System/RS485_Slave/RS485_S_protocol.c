...
//-----------------------------------------------------------------------------------
void upc_buff_init(void)
{
	memset((void *)&rx, 0, sizeof(rx));
	memset((void *)&tx, 0, sizeof(tx));	
}
//-----------------------------------------------------------------------------------
void upc_RXHandler(void)
{	
#if RS232_CONSOLE
	u8 tmp;
	u16 CRC;
	//tmp=ID[3]/10;
	//Serial_ID = ID[3] - tmp*10;
	Serial_ID = ExtCV.ser_id;
//	u8 ltmp;
//	u8 i,old_stat;
	upc_rxd=upc_rx_byte();
//	UDR0=upc_rxd;	
	...
	//			mem_dump(rx.field.d,10);
				switch(rx.field.com.bit.com)
				{
					case com_reqt_status:
						if(rx.field.com.bit.id==Serial_ID)
							upc_answer_sys();	
					break;
					case com_get_stat:
						if(rx.field.com.bit.id==Serial_ID)
					...
					break;
					case com_reqt_reset:
						if((rx.field.com.bit.id==Serial_ID)||(rx.field.com.bit.id==0))
							reset();
					break;					
