...
void con_handle()
{
	...
	{
		in=console_rx_byte();
		if(con_txt[0]==0) // If outbuff is empty, no out any text
		{
			switch(in)
			{
				case 27:			
				...
				if(MenuPtr->Flag==0)				
				{
					...
					if(strcmp3((u8*)MenuPtr->Word,(u8*)word)==0)
					{
						...
						else
						{
							...
							{
								MenuPtr->Routine();						
								__sprintf(con_txt,Invitation_Str);		
							}
							MenuPtr=NULL;			
							break;			
						}
...
//						MenuList_Count++;					
						...
						{
							__sprintf(con_txt,"\n\r ");
							__sprintf(con_txt,StrLine);
							__sprintf(con_txt," : Available parameters ");
							MenuPtr2=(TCMenuItem in_flash*)MenuPtr->SubItem;
							while(MenuPtr2!=NULL)
							{
								__sprintf(con_txt,"\n\r   ");
								//GetFlashStr(con_txt,(u8*)MenuPtr2->Word);
								put_str(con_txt,(u8*)MenuPtr2->Word);
