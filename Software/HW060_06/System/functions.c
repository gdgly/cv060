...
{
#ifdef __IP_SILENCE_RECOVER
	u8 old_id[4];
#ifdef __IP_SILENCE_RESET
	if (restore_ip_core_cycle_delay == 0){
		restore_ip_core_step = 1;
		restore_ip_core_cycle_delay = 5*60;
		restore_ip_core_count ++;
		if (restore_ip_core_count > 5) reset();
	}
#endif
...
//		ID[2]=1;
//		ID[3]=250;
		IP4_ADDR_MAKE(new_ip,192,168,1,250);
