...
//	u16 tmp;
	...
	ShowPowerShort(outtxt,(s16*)&kotel.power);					
	__sprintf_P(outtxt,kotel_dic[26]);			
	WordToStr(outtxt,(u16*)&kotel.t3max,0);
	__sprintf_P(outtxt,kotel_dic[27]);			
	ShowPowerShort(outtxt,(s16*)&kotel.p3);
...
//#else
//	__sprintf_P(outtxt,kotel_dic[28]);			
#endif	
}
//------------------------------------------------------------------------------
void Mod_Show_Kotel_Params()
{
	...
	if((mode>0)&&(mode<5))
	{
		...
		// Temperature
		__sprintf_P(outtxt,kotel_dic[2]);			
		ShowTemperature(outtxt,(s16*)&kotel.p[kotel.fuel][mode].t);
		// Smoke
